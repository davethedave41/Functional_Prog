module W2 where
    ...
    map2 :: (a -> b -> c) -> [a] -> [b] -> [c]
    map2 myf (a:as) (b:bs) = myf a b : map2 myf as bs
    map2 myf _  _  = []
    ...
    interpreter commandstr = go 0 0 commandstr
        where go a b ("incA":commandstr)   = go (a+1) b commandstr 
              go a b ("decA":commandstr)   = go (a-1) b commandstr
              go a b ("printA":commandstr) = show a : go a b commandstr
              go a b ("printB":commandstr) = show b : go a b commandstr
              go a b ("decB":commandstr)   = go a (b-1) commandstr
              go a b ("incB":commandstr)   = go a (b+1) commandstr
              go a b (_:commandstr)        = "BAD" : go a b commandstr
              go a b []                    = []
